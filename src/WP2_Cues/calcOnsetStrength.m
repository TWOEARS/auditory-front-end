function [CUE,SET] = calcOnsetStrength(SIGNAL,CUE)
%calcOnsetStrength   Calculate onset strength in dB. 
% 
%USAGE
%   [CUE,SET] = calcOnsetStrength(SIGNAL,CUE)
%
%INPUT PARAMETERS
...
onset = cat(2,zeros(nFilter,1,nChannels),diff(energy,1,2));

% Discard offsets and limit onset strength
onset = min(max(onset,0),SET.maxOnsetdB);
