...
%% Properties
    ...
    properties (Dependent = true)
        ...
        mocTau              % time constants for MOC attenuation calculation
        mocTauWeights       % weighting to give to each time constant
    ...
    properties (GetAccess = private)
        ...
        mocThresholdRate            % threshold AN rate over which the MOC attenuation will be driven
        minMOCAttenuationdB         % Minimum MOC attenuation in dB
        rateToAttenuationFactor
        mocDecay
        mocFactor
        moc
        mocBoundary
...
%% Methods
    methods
        ...
        function out = processChunk(pObj,in)
            ...
            pObj.LowerDependencies{1}.LowerDependencies{1}.LowerDependencies{1}.modifyParameter('fb_mocIpsi', mocIpsiFeed);
            pObj.LowerDependencies{1}.LowerDependencies{1}.LowerDependencies{1}.modifyParameter('fb_mocContra', mocContraFeed);
...
%         end
    ...
    methods
        ...
        function mocContra = get.mocContra(pObj)
            ...
            mocContra = pObj.parameters.map('moc_mocContra'); % Complete dummy value!
        end
    ...
    methods (Static)
        ...
        function [names, defaultValues, descriptions] = getParameterInfo()
            ...
            names = {'moc_mocIpsi', 'moc_mocContra'};
